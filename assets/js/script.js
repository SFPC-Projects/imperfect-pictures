(function () {
    ...
    function attachWindowControls(overlay, onClose) {
        ...
        const titlebar = windowPanel.querySelector('.titlebar');
        if (titlebar && windowPanel.classList.contains('floating')) {
            ...
            function getWindowPanelPosition(el) {
                return {
                    x: Number(el.dataset.x || 0),
                    y: Number(el.dataset.y || 0)
                };
            }
            function setWindowPanelPosition(el, x, y) {
                el.style.transform = `translate(${x}px, ${y}px)`;
                el.dataset.x = String(Math.round(x));
                el.dataset.y = String(Math.round(y));
            }
            ...
            function onPointerDown(e) {
                if (windowPanel.classList.contains('maximized')) return;
                if (e.pointerType === 'mouse' && e.button !== 0) return;
                if (!e.target.closest('.titlebar') || e.target.closest('.titlebar-controls')) return;
                dragActive = false;
                pointerId = e.pointerId;
                const pos = getWindowPanelPosition(windowPanel);
            ...
            function onPointerMove(e) {
                ...
                if (!dragActive) {
                    if (Math.abs(dx) >= DRAG_THRESHOLD || Math.abs(dy) >= DRAG_THRESHOLD) {
                        dragActive = true;
                        suppressClick = true;
                        try { windowPanel.setPointerCapture(e.pointerId); } catch { }
                        bringToFront(overlay);
                    } else {
                        return;
                    }
            ...
            function onPointerUp(e) {
                if (pointerId !== e.pointerId) return;
                try { windowPanel.releasePointerCapture(e.pointerId); } catch { }
                dragActive = false;
                pointerId = null;
            ...
            function onClickCapture(e) {
                if (!suppressClick) return;
                if (windowPanel.contains(e.target)) {
                    e.stopPropagation();
                    e.preventDefault();
                }
                suppressClick = false;
